/*global s d3, sitemap, _, d3, options*/

(function () {

  sitemap.pageMenuItem = function (className) {
    var item = {};
    ...
    function drag (parentNode) {
      return sitemap.addDrag(className + '-drag')
          .start(function (d) {
            parentNode.datum(function(x) {
              if (x.id === d.pid) {
                clearTimeout(x.page.menuTimeout);
  ...
  var buttonView = function (name, transform) {
    return sitemap.D3View(function (svg) {
      var removeItem = sitemap.pageMenuItem(name)
        .transform(transform);
  ...
  sitemap.addAfterButtonView = buttonView('add-after-button',
                                            function () {
  ...
  sitemap.pageMenuView = sitemap.D3View(function (svg) {
